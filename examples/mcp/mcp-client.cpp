...
public:
    ...
    json send_request(const json& request) {
        ...
        fflush(server_stdin);
        std::cout << "Request sent, waiting for response..." << std::endl;  // Add this line
...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        std::cout << "DEBUG: start_server returned successfully" << std::endl;  // Add this
        
        // Give server time to start and check for any immediate logs
        std::this_thread::sleep_for(std::chrono::milliseconds(200));
        client.read_server_logs();
        
        // Step 1: Initialize
        print_separator("INITIALIZING");
        std::cout << "About to send initialize request..." << std::endl;
        ...
        std::cout << "Initialized notification sent" << std::endl;
        
        // Give server time to process and check logs
        std::this_thread::sleep_for(std::chrono::milliseconds(100));
        client.read_server_logs();
        ...
        // Step 4: Call transcribe tool
        print_separator("CALLING TRANSCRIBE TOOL");
        json transcribe_args = {
            {"file", "samples/jfk.wav"}
        };
        
        json inference_response = client.call_tool("transcribe", transcribe_args);
        std::cout << "Transcribe response:" << std::endl;
        pretty_print_json(inference_response);
        
        // Step 5: Call server stats
        /*
        print_separator("CALLING SERVER STATS TOOL");
        json stats_response = client.call_tool("model_info", json::object());
        std::cout << "Server stats response:" << std::endl;
        pretty_print_json(stats_response);
        
        print_separator("SECOND TRANSCRIBE CALL");
        json transcribe_args2 = {
            {"file", "samples/jfk.wav"}
        };
        
        json inference_response2 = client.call_tool("transcribe", transcribe_args2);
        std::cout << "Second transcribe response:" << std::endl;
        pretty_print_json(inference_response2);
        */
