...
USER_NAME = "danbev"
REPO_ID   = f"{USER_NAME}/nemotron-asr-streaming-GGUF"

MODELS = {
    "f32": {
        "local_path":   "models/ggml-nemotron-3.5-asr-streaming-0.6b-f32.bin",
        "remote_name":  "ggml-nemotron-3.5-asr-streaming-0.6b-f32.bin",
        "description":  "Full precision (F32)",
    },
    "f16": {
        "local_path":   "models/ggml-nemotron-3.5-asr-streaming-0.6b-f16.bin",
        "remote_name":  "ggml-nemotron-3.5-asr-streaming-0.6b-f16.bin",
...
def build_model_card(uploaded_variants):
    lines = [
        ...
        f"---",
        f"",
        f"#  Nemotron 3.5 ASR (GGUF)",
        f"",
        f"GGUF conversions of [nvidia/nemotron-3.5-asr-streaming-0.6b](https://huggingface.co/nvidia/nemotron-3.5-asr-streaming-0.6b) for use with [whisper.cpp](https://github.com/ggml-org/whisper.cpp).",
        f"",
        f"## Available files",
        f"",
    ]
    ...
    lines += [
        ...
        f"Run:",
        f"```console",
        f"./build/bin/parakeet-cli -m models/{MODELS['f16']['remote_name']} -f samples/jfk.wav",
        f"```",
        f"",
    ]
